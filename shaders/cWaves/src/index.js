...
float cnoise(vec3 P){
  ...
  return 2.2 * n_xyz;
    ...
    void main() {
        ...
        elevation += cnoise(vec3(modifiedPosition.xy, Time * 0.3));
...
varying float vEleve;
    ...
    void main(){ 
        vec3 color = mix(depthColor, faceColor, vEleve *3.0 + 1.0);
...
// GUI controls
const gui = new GUI();
gui.add(shader.uniforms.waveFrequancy, 'value').min(0.0).max(1.0).step(0.001).name('waveFrequancy');
gui.add(shader.uniforms.wavesFrequancy.value, 'x').min(0.0).max(5.0).step(0.1).name('wavesFrequancyX');
gui.add(shader.uniforms.wavesFrequancy.value, 'y').min(0.0).max(5.0).step(0.1).name('wavesFrequancyY');
